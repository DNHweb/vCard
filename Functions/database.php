<?php
    ...
    function configuration_get_value($name)
    {
        global $databaseConnection;

        $query = 'SELECT `value` FROM `configuration` WHERE `name` = "' . $name . '" LIMIT 1';
        $result = $databaseConnection->query($query)->fetch_object()->value;
